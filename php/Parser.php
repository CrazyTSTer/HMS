    ...
    public static function parseMonth($data, $currentDate = null, $isLast12Month = false)
    {
        if ($data == false) {
            ...
            for ($i = 1; $i < $data[DB::MYSQL_ROWS_COUNT]; $i++) {
                ...
                $ret['data'][self::TIMESTAMP][0][] = $isLast12Month ? strftime('%b. %Y', $ts) : strftime('%e %b. (%a)', $ts);
            ...
            if (!is_null($currentDate)
                ...
                $ret['data'][self::TIMESTAMP][0][] = $isLast12Month ? strftime('%b. %Y', strtotime($currentDate)) : strftime('%e %b. (%a)', strtotime($currentDate));

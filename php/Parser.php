    ...
    public static function parseMonth($data, $currentDate = null, $isLast12Month = false)
    {
        if ($data == false) {
            ...
            for ($i = 1; $i < $data[DB::MYSQL_ROWS_COUNT]; $i++) {
                ...
                $ret['data'][self::TIMESTAMP][0][] = $isLast12Month ? ucfirst(strftime('%b. %Y', $ts)) : ucfirst(strftime('%e %b. (%a)', $ts));
            ...
            if (!is_null($currentDate)
                ...
                $ret['data'][self::TIMESTAMP][0][] = $isLast12Month ? ucfirst(strftime('%b. %Y', strtotime($currentDate))) : ucfirst(strftime('%e %b. (%a)', strtotime($currentDate)));
